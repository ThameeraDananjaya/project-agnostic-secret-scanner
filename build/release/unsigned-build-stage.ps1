...
& /usr/bin/aa-exec --profile pscan-native-diagnostic -- /opt/microsoft/powershell/7/pwsh -NoLogo -NoProfile -NonInteractive -File "$PSScriptRoot/unsigned-build-profile-driver.ps1" -Stage $Stage -BaselinePath $baseline -BaselineSHA256 $baselineHash -Mode $Mode
