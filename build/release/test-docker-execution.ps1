...
'@
            ...
            function LinuxCaptureTypes($r){if($r.StdOut-isnot[byte[]]-or$r.StdErr-isnot[byte[]]){throw 'Native fixture capture lost its byte array type'}}
            function LinuxSuccess($r,[string]$name){LinuxCaptureTypes $r;if($r.Terminal-or!$r.ContainmentEmpty){[Console]::Error.WriteLine((ConvertTo-PscanNativeFixtureDiagnostic -Result $r -CaseName $name));throw "$name returned terminal lifecycle evidence"}}
            function LinuxTerminal($r,[string]$name){LinuxCaptureTypes $r;if(!$r.Terminal-or$r.Terminal.Contains('cleanup uncertainty')){[Console]::Error.WriteLine((ConvertTo-PscanNativeFixtureDiagnostic -Result $r -CaseName $name));throw "$name did not prove expected terminal cleanup"}}
