...
func NewCosignCommandVerifier(path, expectedSHA256 string) (*CosignCommandVerifier, error) {
	if !IsDigest(expectedSHA256) {
	...
	return &CosignCommandVerifier{path: path, sha256: expectedSHA256}, nil
}
...
func (v *CosignCommandVerifier) VerifyManifest(ctx context.Context, manifestPath, bundlePath string, identity ReleaseIdentity) error {
	...
	refreshed, err := NewCosignCommandVerifier(v.path, v.sha256)
