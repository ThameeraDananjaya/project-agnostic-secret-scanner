...
TAG = "release-tooling-v1.0.0-c2-linux-build-v2"
...
schema-release-manifest-2.1.json schema-release-manifest-2.2.json schema-release-manifest-2.3.json schema-release-manifest-2.4.json
...
def validate_distribution(directory, revision):
    ...
    require(len(entries) == 34 and {p.name for p in entries} == FILES, "DISTRIBUTION_FILE_SET")
    sizes = {}
    for p in entries:
        no_links(p)
        s = p.lstat()
    ...
    require(isinstance(manifest, dict) and manifest.get("manifestSchemaVersion") == "2.4" and
    ...
    require(isinstance(assets, list) and len(assets) == 33, "MANIFEST_ASSETS")
    seen = set()
    for asset in assets:
...
def check(directory, raw_admission, revision, now):
    ...
    return {"schema": "pscan-build-storage-guard-result-v1", "status": "PASS",
            "tooling_revision": revision, "files": 34, "payload_bytes": total,
