...
6. The gated declared-maximum fixture passed an actual network-disabled
   Gitleaks run at exactly 512 MiB, then passed again after review corrections
   in 128.38 seconds. Profile unit tests reject
...
- Raw classification calls every independently registered archive/compression
  family in the same `github.com/mholt/archives` v0.1.2 stack and the exact
  `github.com/h2non/filetype` v1.1.3 application-MIME matcher used by the pinned
  detector. Archive, compression, application/container and ambiguous results
  are non-pass before preparation; ordinary text and binary are admitted.
